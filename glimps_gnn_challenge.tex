...
\section*{A.4 C04: GLIMPS-GNN}

\subsection*{A.4.1 Competition Overview}
...
\subsection*{A.4.2 Dataset Description}
...
\subsection*{A.4.3 Input Files}

Organizers provide tabular CSVs and serialized PyG artifacts:
\begin{itemize}
    \item \texttt{train.csv} / \texttt{test.csv} --- Node features and labels (labels withheld for test);
    \item \texttt{adjacency\_matrix.csv} --- Dense $\mathbf{A} \in \{0,1\}^{320 \times 320}$;
    \item \texttt{graph\_edges.csv} --- Sparse COO edge list with \texttt{src}, \texttt{dst}, \texttt{edge\_type};
    \item \texttt{node\_types.csv} --- Node ID registry aligning $\mathbf{A}$, $\mathbf{X}$, and edges;
    \item \texttt{graph\_artifacts.pt} --- Pre-built PyTorch Geometric \texttt{HeteroData} objects.
\end{itemize}
...
\subsection*{A.4.4 Submission and Evaluation}

The competition uses an automated GitHub Actions pipeline:
\begin{enumerate}
    \item \textbf{Generate} a \texttt{predictions.csv} with columns \texttt{id} and \texttt{y\_pred} (probability or hard label);
    \item \textbf{Encrypt} it with the organizer GPG public key to produce \texttt{predictions.csv.enc};
    \item \textbf{Submit} the encrypted file plus \texttt{metadata.json} (team, run, model name, type, submitter) via pull request;
    \item \textbf{Auto-score:} the workflow decrypts, validates format, scores against hidden labels, posts metrics, updates the leaderboard, and closes the PR.
\end{enumerate}
